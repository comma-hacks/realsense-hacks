...
int main(int argc, char **argv)
{
  ...
  while (true)
  {
      ...
      if (position_error.x() > 0.01) robotMoveBackward();
      else if (position_error.x() < -0.01) robotMoveForward();
      if (position_error.y() > 0.01) robotRotateLeft();
      else if (position_error.y() < -0.01) robotRotateRight();

      // Apply inputs to the robot's web API based on the rotation error
      if (yaw_error > 0.01) robotRotateLeft();
      else if (yaw_error < -0.01) robotRotateRight();
